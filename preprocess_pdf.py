...
# Change the metadata to include the document name
# def create_indexing(data, index, file_name):
#     batch_limit = 100
#     texts = []
#     meta_datas = []
#     text_splitter = RecursiveCharacterTextSplitter(chunk_size=200, chunk_overlap=50, length_function=tiktoken_len,
...
#         record_metadata = [{"document": file_name, "chunk": j, "text": text, **metadata} for j, text in enumerate(record_texts)]
...
def vectors_store(data, file_name=""):
    index_name = "coms579pdfqa"
    existing_indexes = [
    ...
    # create_indexing(data, index, file_name)
...
def upload_index_pdf_to_pinecone(file_path):
    ...
    loader, data, file_name = pdf_loader(file_path)
    vectors_store(data)
...
def command_running():
    while True:
        ...
        loader, data, file_name = pdf_loader(pdf_path)
        ids = vectors_store(data)
        # ids = vectors_store(data, file_name)
